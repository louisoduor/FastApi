...
def create_employee():
    ...
    hashed_password = generate_password_hash(data['password'], method='ars1234')

    new_employee = Employee(public_id=str(uuid.uuid4()), name=data['name'], password=hashed_password, admin=False)
    employee = Employee(
        public_id=fake.uuid4(),
        name=data['name'],
